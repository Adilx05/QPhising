...
- 2026-06-07: Added 6 production-grade xUnit test files for Campaign and Template features (`CampaignDomainUnitTests.cs`, `CampaignApplicationUnitTests.cs`, `TemplateDomainUnitTests.cs`, `TemplateApplicationUnitTests.cs`, `CampaignEndpointsIntegrationTests.cs`, `TemplateEndpointsIntegrationTests.cs`) covering domain invariants, lifecycle transitions, value objects, CQRS handler/validator behavior, role-based authorization, and endpoint integration flows — all compile with 0 errors, 239 tests total (237 pass).
- 2026-06-07: Added 5 frontend Jasmine/Karma spec files (`oidc-auth.service.spec.ts`, `auth-session.spec.ts`, `authentication.guard.spec.ts`, `api-error-handler.spec.ts`, `openapi-config.provider.spec.ts`) covering OIDC PKCE flow, JWT role/permission resolution, route guards, API error handling with ProblemDetails parsing, and OpenAPI config/axios interceptor behavior — 76 total tests passing.
- 2026-06-07: Cleaned up stale `backend/API.IntegrationTests/` directory (dead build artifacts, no `.csproj` or source code) and removed empty `backend/tests/.gitkeep` placeholder.
...
- 2026-06-07: Added 6 production-grade xUnit test files for Campaign and Template features (`CampaignDomainUnitTests.cs`, `CampaignApplicationUnitTests.cs`, `TemplateDomainUnitTests.cs`, `TemplateApplicationUnitTests.cs`, `CampaignEndpointsIntegrationTests.cs`, `TemplateEndpointsIntegrationTests.cs`) covering domain invariants, lifecycle transitions, value objects, CQRS handler/validator behavior, role-based authorization, and endpoint integration flows — all compile with 0 errors.
- 2026-06-05: Added `AllowScriptExecution` boolean to `PageSettings` domain model (default `true`), propagated through `TrackingPageAggregate`, persistence mapping, repository, CQRS commands/handlers, API DTOs, controller, and response contracts. Added `allowScriptExecution` checkbox to campaign create form and edit modal, conditionally applies `sandbox` on public landing page iframe when disabled. Added `campaign.update` action to `AuditEventTaxonomy` for PUT `/api/campaigns/{id}` audit logging. Added EF Core migration note: requires `dotnet ef migrations add AddAllowScriptExecutionToTrackingPages`.
