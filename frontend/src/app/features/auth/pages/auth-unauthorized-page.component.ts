...
@Component({
  ...
  template: `
    <section class="mx-auto max-w-xl rounded-2xl border border-rose-200 bg-white p-8 shadow-sm">
      <h1 class="text-xl font-semibold text-rose-700">{{ tx('Erişim reddedildi', 'Access denied') }}</h1>
      <p class="mt-3 text-sm text-slate-600">
        {{ tx('Kimliğiniz doğrulandı ancak bu alan için gerekli role sahip değilsiniz.', 'You are authenticated but do not have the required role for this area.') }}
      </p>
      <p *ngIf="reason" class="mt-2 text-xs text-slate-500">{{ tx('Neden', 'Reason') }}: {{ reason }}</p>
      ...
      <div class="mt-6 flex gap-3">
        <button
          ...
          (click)="signInAgain()">
          {{ tx('Başka bir hesapla giriş yap', 'Sign in with another account') }}
        </button>
        ...
        <button
          ...
          (click)="goToDashboard()">
          {{ tx('Gösterge paneline dön', 'Back to dashboard') }}
        </button>
      </div>
    </section>
  `
